...
const DishDetail = ({ id, imgUrl, isVeg, title, price, avgRating, description = "", restaurantName }) => {
  ...
  const handleAdd = () => addToCart({ id, imgUrl, isVeg, title, price }, restaurantName);
        ...
        <div className="relative">
          ...
          <div className="absolute bottom-16 right-1 md:right-5 md:-bottom-[12px]">
            {quantity === 0 ? (
              <button
                onClick={handleAdd}
                className="text-green-600 bg-white px-11 font-bold py-2 border shadow-xl border-green-600 rounded-lg hover:bg-gray-100 transition-colors"
              >
                ADD
              </button>
            ) : (
              <div className="flex items-center bg-white border border-green-600 rounded-lg shadow-xl overflow-hidden">
                <button
                  onClick={handleDecrease}
                  className="text-green-600 font-bold px-3 py-2 hover:bg-gray-100 transition-colors text-lg"
                >
                  −
                </button>
                <span className="text-green-600 font-bold px-3">{quantity}</span>
                <button
                  onClick={handleAdd}
                  className="text-green-600 font-bold px-3 py-2 hover:bg-gray-100 transition-colors text-lg"
                >
